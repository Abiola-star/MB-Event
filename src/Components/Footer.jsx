...
export default function Footer() {
  ...
  const handleSubmit=async (e)=>{
    ...
    try {
      ...
      if (response.status !== 200) throw new Error(data.message || "Subscription failed");
  ...
  transition={{ duration: 0.3 }} className="container lg:flex mx-auto items-center justify-between">
         ...
         <div className="branding max-w-[280px]">
            ...
            <form onSubmit={handleSubmit} className="flex w-xs lg:w-[350px] bg-white rounded-md mt-4 py-2 px-2">
                ...
                <button className="w-[150px] lg:w-[200px] bg-purple-600 rounded-md" type="submit"> subscribe</button>
            </form>
            {errors.email && <span className="text-red-500 font-semibold  w-md" >{errors.email}</span>}
        </div>
